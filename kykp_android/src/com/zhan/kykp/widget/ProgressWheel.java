...
public class ProgressWheel extends View {
    ...
    //wuyue
    private int paddingTextLine = 16;
    private int unitPadding = 16;
    ...
    //wuyue
    private int summaryColor = 0xFF000000;
    ...
    //wuyue
    private Paint summaryPaint = new Paint();
    ...
    //wuyue
    private RectF progressCircleContour = new RectF();
    ...
    private void setupPaints() {
        ...
        //wuyue
        summaryPaint.setColor(summaryColor);
        summaryPaint.setStyle(Style.FILL);
    ...
    private void setupBounds() {
        ...
        //wuyue
        progressCircleContour = new RectF(circleBounds.left + rimWidth  + (contourSize / 2.0f), circleBounds.top + rimWidth  + (contourSize / 2.0f), circleBounds.right - rimWidth - (contourSize / 2.0f), circleBounds.bottom - rimWidth - (contourSize / 2.0f));
    ...
    private void parseAttributes(TypedArray a) {
        ...
        //Start_wuyue
        summarySize = (int) a.getDimension(R.styleable.ProgressWheel_summarySize,
        		summarySize);
        
        summaryColor = (int) a.getColor(R.styleable.ProgressWheel_summaryColor,
        		summaryColor);
        
        paddingTextLine = (int) a.getDimension(R.styleable.ProgressWheel_textLinePadding,
        		paddingTextLine);
        
        if (a.hasValue(R.styleable.ProgressWheel_unit)) {
        	unit=a.getString(R.styleable.ProgressWheel_unit);
        }
        
        unitPadding = (int) a.getDimension(R.styleable.ProgressWheel_unit_padding,
        		16);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //Draw the bar
        //wuyue switch circleBounds to progressCircleContour
        if (isSpinning) {
            canvas.drawArc(progressCircleContour, progress - 90, barLength, false,
                    barPaint);
        } else {
            canvas.drawArc(progressCircleContour, -90, progress, false, barPaint);
        }
        ...
        //summary !=null 上移一个summary +paddingTextLine
		FontMetrics fm = summaryPaint.getFontMetrics();
		int summaryHeight = (int) (Math.ceil(fm.descent - fm.ascent) + 2);
        if (!TextUtils.isEmpty(summary)) {
        	startY-=summaryHeight/2;
        }
        //End_wuyue
        
        for (String s : splitText) {
            float horizontalTextOffset = textPaint.measureText(s) / 2;
            canvas.drawText(s, getWidth() / 2 - horizontalTextOffset,
            		startY + verticalTextOffset, textPaint);
        }
        
        //Start_wuyue draw Unit
		if (!TextUtils.isEmpty(unit)) {
			float unitStartX = getWidth() / 2 + textPaint.measureText(text) / 2 + unitPadding;
			canvas.drawText(unit, unitStartX, startY + verticalTextOffset, summaryPaint);
		}
        
		if (!TextUtils.isEmpty(summary)) {
			float y  = startY+ verticalTextOffset;
			for (String s : splitSummary) {
				y += summaryHeight / 2 + paddingTextLine;
				float horizontalTextOffset = summaryPaint.measureText(s) / 2;
				canvas.drawText(s, this.getWidth() / 2 - horizontalTextOffset, y, summaryPaint);
			}
		}
		//End_wuyue
    ...
    */
    ...
    public void setSummary(String summary){
    	...
    	splitSummary= this.summary.split("\n");
    }
